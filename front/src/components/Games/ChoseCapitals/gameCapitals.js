...
export default class GameCapitals extends Component {
    ...
    state = {
     ...
     componentDidMount(){
         console.log('hijo de puta',this.context)
     }
    getCountry = () => {
        let arrayCountries = []
    ...
    chooseCapital = (e) => {
        ...
        if (this.state.countries[0].capital === e.target.id) {
            ...
            Shuffle(fourCapitals)
            this.setState({
                //When the answer is right: update countries and capitals, sum seconds and score and hide incorrect message
    ...
    render() {
        ...
        return ( //"City Game pone a prueba tus conocimientos de Geografia, relaciona el País con su capital, Si aciertas ganas 10 puntos y 4 segundos y si fallas restas 3s."
            <MyContext.Consumer>
            {(context)=>(
                <Fragment>
                    <InstructionGames changeClick={()=> context.changeScore(22320)} instructionText={context.state.user}/>
                    <CloseButton />
